...
const hasChildrenEntries = (arr, character) => {
  ...
  arr.forEach(obj => {
    if (
      Object.entries(obj)[0][1] === character &&
      Object.entries(obj)[2][1].length > 0
    ) {
      result = true;
    }
  });
...
const totalCharacters = (arr) => {
  ...
  arr.forEach(character => {
    if (!uniqueCharacters.includes(character.name) && character.name) { uniqueCharacters.push(character.name); }
    if (!uniqueCharacters.includes(character.spouse) && character.spouse) { uniqueCharacters.push(character.spouse); }
    character.children.forEach(child => {
      if (!uniqueCharacters.includes(child)) { uniqueCharacters.push(child); }
    });
  });
