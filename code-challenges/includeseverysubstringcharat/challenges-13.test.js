...
const firstLetters = (arr) => {
  let newArr = [];
  arr.forEach(string => newArr.push(string.split('')[0]));
  return newArr;
};
...
const findHappiness = (arr) => {
  let newArr = [];
  arr.forEach(string => string.includes(':)') ? newArr.push(string) : newArr = newArr);
  return newArr;
};
...
const standardizePhoneNumbers = (arr) => {
  ...
  arr.forEach(string => {
    ...
    fixed = fixed.filter(char => char !== ')' && char !== '(' && char !== ' ' && char !== '-');
    console.log(fixed);
    newArr.push(fixed.join(''));
  });
  return newArr;
};
...
const onlyOddChars = (str) => {
  return str.split('').filter((v, i) => i % 2 !== 0).join('');
};
...
const allHappy = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (!arr[i].includes(':)')) { answer = false; }
  }
  return answer;
};
...
const findEvery = (arr, target) => {
  let answer = true;
  arr.forEach(string => {
    if (!string.includes(target)) { answer = false; }
  });
  return answer;
};
...
const unenrollBrook = (arr) => {
  let newArr = [];
  for (let i = 0; i < arr.length; i++) {
    let fixed = arr[i].filter(string => string.includes('Brook') === false);
    newArr.push(fixed);
  }
  return newArr;
};
