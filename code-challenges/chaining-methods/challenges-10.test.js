...
const count = (target, input) => {
  return input.reduce((count, value) => {
    //here, value is an array in the input array
    return count + value.reduce((count, value) => {
      return value === target ? count + 1 : count + 0;
    }, 0);
  }, 0);
};
...
let findMaleAndFemale = (data) => {
  return data.filter(value => value.gender === 'male' || value.gender === 'female')
  .reduce((string, obj) => {
    return string + `${obj.name} and `;
  }, '')
  .slice(0, -5);
};
...
let findShortest = (data) => {
  return data.reduce((name, obj) => {
    if (obj.name.length > name.length) {
      return name;
    } else {
      return obj.name;
    }
