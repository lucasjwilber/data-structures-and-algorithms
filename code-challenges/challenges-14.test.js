...
const toTitleCase = (arr) => {
  return arr.map(string => string = (string[0].toUpperCase() + string.slice(1)))
};
...
let biggerThanLuke = (arr) => {
  ...
  let baseline = 0;
  arr.forEach(obj => {
    if (obj.name === 'Luke Skywalker') { baseline = +obj.mass; }
  });
  arr.forEach(obj => {
    if (+obj.mass > baseline) { newArr.push(obj.name); }
  });
  return newArr.join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i = 0; i < board.length; i++) {
    if (board[i][0] === board[i][1] && board[i][0] === board[i][2] && board[i][0] !== '') { answer = true;}
    if (board[0][i] === board[1][i] && board[0][i] === board[2][i] && board[0][i] !== '') { answer = true;}
  }
  if (board[0][0] === board[1][1] && board[0][0] === board[2][2] && board[0][0] !== '') { answer = true; }
  if (board[0][2] === board[1][1] && board[0][2] === board[2][0] && board[0][2] !== '') { answer = true; }
  return answer;
};
