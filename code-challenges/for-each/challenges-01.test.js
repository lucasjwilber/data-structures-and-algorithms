...
//use a forEach with a function as an argument which has parameters (element, index, array) and call it on every index of the array. 
//it should pop off every element that is (%3 ===2)


const removeWithAnon = (arr) => {

  arr.forEach((value, index, array) => {
    if (value % 3 === 2) { array.pop(); }
  });
  return arr;
...
const fizzbuzz = (arr) => {
  let output = [];
  arr.forEach(value => {
    if (value % 15 === 0) { output.push('Fizz Buzz'); }
